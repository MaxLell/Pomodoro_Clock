...
STATIC void ContextManagement_IdleStateFunction(void)
{
    ...
    // Only execute this section, when the Trigger Btn or the Rotary Encoder Btn was pressed
    if ((sButtonMessage.eButton == E_BTN_TRIGGER) && (sButtonMessage.eEvent == E_BTN_EVENT_SHORT_PRESSED))
    {
        ...
        if (E_BTN_TRIGGER == sButtonMessage.eButton)
        {
            // reset the RunOnce Flag
            bRunOnce = false;

            // Reset the BTN back to invalid
            sButtonMessage.eButton = E_BTN_INVALID;
...
STATIC void ContextManagement_PomodoroStateFunction(void)
{
    ...
    // The main action is happening in the Idle State Function
}
...
status_e ContextManagement_MsgCallback(const msg_t *const in_sMsg)
{
    ...
    case MSG_0103:
    {
        ...
        ASSERT_MSG(!(sButtonMessage.eButton == E_BTN_INVALID), "Invalid Button Type: %d", sButtonMessage.eButton);
