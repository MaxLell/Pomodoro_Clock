...
void FSM_checkConfig(const FSM_Config_t *const in_psFsmConfig)
{
    ...
    ASSERT_MSG(!(in_psFsmConfig->au16TransitionMatrix == NULL), "Transition Matrix is NULL");
    ASSERT_MSG(!(in_psFsmConfig->asStateActions == NULL), "State Actions Array is NULL");
...
void FSM_setTriggerEvent(FSM_Config_t *const inout_psFsmConfig, uint16_t in_u16Event)
{
    { // Input Checks
        ...
        ASSERT_MSG(!(in_u16Event >= inout_psFsmConfig->NUMBER_OF_EVENTS), "Event is not valid");
    }
...
void FSM_getNextState(FSM_Config_t *const inout_psFsmConfig)
{
    ...
    inout_psFsmConfig->u16CurrentState = *(au16Matrix + (u16State * u16NofEvents) + u16Event);

    ASSERT_MSG(!(inout_psFsmConfig->u16CurrentState >= inout_psFsmConfig->NUMBER_OF_STATES),
