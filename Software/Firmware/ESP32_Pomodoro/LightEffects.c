...
{
    // Input Checks
    ASSERT_MSG(!(u8Idx >= TOTAL_LEDS), "u8Idx is larger than RGB_LED_TOTAL_LEDS");
                                 ...
                                 bool bIsWorktimeOver)
{
    ...
    for (u8 i = 0; i < in_u8ArraySize; i++)
    {
        ...
        case E_CFG_WORKTIME:
            ...
            ASSERT_MSG(false == bIsWorktimeOver,
...
void LightEffects_RenderSeekingAttention(LightEffects_SeekingAttention_s *const psSelf)
{
    ...
    if (false == psSelf->bInitialized)
    {
        ...
        HelperFunction_getPseudoRandomNumber(START_INDEX_OUTER_RING, END_INDEX_MIDDLE_RING, (u32 *)&psSelf->u8CurrentLedIdx);
...
void LightEffects_RenderScore(u32 in_u32ScoreInMinutes)
{
    ...
    {
        for (u32 i = START_INDEX_INNER_RING; i < START_INDEX_INNER_RING + NOF_LEDS_INNER_RING; i++)
        {
            RgbLed_setPixelColor(i, 0, 0, 50);
        }
    }
